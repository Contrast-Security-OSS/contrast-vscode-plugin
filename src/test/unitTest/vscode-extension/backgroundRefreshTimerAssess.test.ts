import { startBackgroundTimer } from '../../../vscode-extension/cache/backgroundRefreshTimer';
import {
  DateTime,
  getCacheFilterData,
...
describe('Background Timer Tests', () => {
  ...
  describe('startBackgroundTimer', () => {
    it('should start the timer and refresh cache based on the cycle time', async () => {
      ...
      const mockGetDataOnlyFromCacheAssess =
        getDataOnlyFromCacheAssess as jest.Mock;
      mockGetDataOnlyFromCacheAssess.mockResolvedValue({
  ...
  describe('resetBackgroundTimer', () => {
    it('should stop the current timer, clear cache, and restart the timer', async () => {
      ...
      await resetBackgroundTimerAssess(projectId);

      expect(mockClearInterval).toHaveBeenCalledTimes(0);
      expect(mockClearCacheByProjectId).toHaveBeenCalledWith(
        'assess-' + projectId
      );
  ...
  describe('getVulnerabilitiesRefreshCycle', () => {
    ...
    it('should return immediately when persist.code is 400', async () => {
      const mockRefreshCycle = 10;
      (getVulnerabilitiesRefreshCycle as jest.Mock).mockResolvedValue(
        mockRefreshCycle
      );
      (getCacheFilterData as jest.Mock).mockResolvedValue({ code: 400 });
      ...
      const intervalCallback = mockSetInterval.mock.calls[0][0];
      const result = await intervalCallback();

      // Verify that refreshCacheAssess was not called
      expect(mockRefreshCacheAssess).not.toHaveBeenCalled();
      // Verify the function returned the persist object with code 400
